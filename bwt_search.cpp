...
bwtSearchResult_t BWT_LocalSearch(uint8_t* seq, int start, int stop, int64_t L_Boundary, int64_t R_Boundary)
{
	int i, pos, p, min_dist;
	bwtintv_t ik, uk, ok[4];
	vector<bwtint_t> LocVec;
	bwtint_t gPos, tk[4], tl[4];
	bwtSearchResult_t bwtSearchResult;
	...
	for (pos = start + 1; pos < stop; pos++)
	{
		...
		{
			if (ok[i].x[2] > 1 && ok[i].x[2] < 100)
			{
				// keep this status
				uk = ok[i];
				bwtSearchResult.len = pos - start + 1;
			}
			ik = ok[i];
		}
	}
	...
	{
		min_dist = MaxIntronSize;
		for (i = 0; i < (int)uk.x[2]; i++)
		{
